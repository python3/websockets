"""
:mod:`websockets.protocol` handles WebSocket control and data frames per
:`sections 4 to 8 of RFC 6455`_.
...
class WebSocketCommonProtocol(asyncio.StreamReaderProtocol):
    """
    Provide common parts of the WebSocket protocol.

    :class:`WebSocketCommonProtocol` assumes that the WebSocket connection is
    established. The handshake is managed in subclasses such as
    ...
    attributes for connections handlers that need them.
    ...
    * the request and response HTTP headers are available in the
    ...
    async def recv(self) -> Data:
        ...
        :raises :raises ~websockets.exceptions.ConnectionClosed: when the
    ...
    async def send(
        ...
        Strings (:class:`str`) are sent as text frames. Bytes-like objects
        (:class:`bytes`, :class:`bytearray`, or :class:`memoryview`) are sent
        as binary frame.

        :meth:`send` also accepts an iterable or an asynchronous iterable of
        strings or bytes-like objects. In that case the message is fragmented.
